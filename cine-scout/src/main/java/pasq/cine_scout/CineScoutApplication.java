...
public class CineScoutApplication {

	public static void main(String[] args) {
		SpringApplication.run(CineScoutApplication.class, args);
	}
	...
	CommandLineRunner run(RoleRepository roleRepository, UserRepository userRepository, PasswordEncoder encoder){
		return args->{
			...
			roles.add(adminRole);
			ApplicationUser admin = new ApplicationUser(1,"admin@gmail.com", "admin", encoder.encode("password"), roles);
